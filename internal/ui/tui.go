...
func NewTUI(version string, debug bool) *TUI {
	...
	tui.navigator = NewNavigator(tui)
	tui.focusManager = NewFocusManager(tui)
	tui.mouseCoordinator = NewMouseCoordinator(tui)
	tui.keyboardHandler = NewKeyboardHandler(tui, tui.navigator, tui.focusManager)
	tui.mouseHandler = NewMouseHandler(tui, tui.navigator, tui.focusManager, tui.mouseCoordinator)
...
func (t *TUI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.MouseMsg:
		return t.mouseHandler.Handle(msg)

	case tea.KeyMsg:
		return t.keyboardHandler.Handle(msg)
...
Commands:
  ?          Toggle help  
  enter      Show details OR view secret data (in secrets tab)
