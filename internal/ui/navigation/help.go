...
// generateHelpContent generates mode-specific help content
func (h *HelpComponent) generateHelpContent(registry *KeybindingRegistry) string {
	var content strings.Builder
	
	// Get bindings for current mode
	bindings := registry.GetAllBindings()
	if len(bindings) == 0 {
		content.WriteString("No keybindings available for current mode.")
		return content.String()
	}
	
	// Group bindings by category
	categories := h.categorizeBindings(bindings)
	...
	for category, categoryBindings := range categories {
		...
		// Category header
		categoryHeader := h.CategoryStyle.Render(category)
...
// categorizeBindings groups bindings by logical categories
func (h *HelpComponent) categorizeBindings(bindings map[string]KeyBinding) map[string][]KeyBinding {
	...
	for _, binding := range bindings {
		...
		case ActionQuit, ActionToggleHelp, 
			 ActionToggleDebug, ActionRefresh, ActionEscape:
		...
		case ActionMoveUp, ActionMoveDown, 
			...
			categories["Movement"] = append(categories["Movement"], binding)
			
		case ActionNextPanel, ActionPrevPanel,
			 ActionNextTab, ActionPrevTab,
			 ActionFocusMain, ActionFocusDetail, ActionFocusLog:
			categories["Navigation"] = append(categories["Navigation"], binding)
			
		case ActionSelect, ActionDelete, ActionEdit,
			 ActionCopy, ActionToggleCollapse, ActionToggleVisible:
			categories["Content"] = append(categories["Content"], binding)
			
		case ActionEnterSearch, ActionEnterCommand,
			 ActionEnterInsert, ActionEnterNormal:
			categories["Mode Switching"] = append(categories["Mode Switching"], binding)
			
		case ActionClearLogs, ActionToggleAutoscroll,
			 ActionTogglePause, ActionFilterLevel:
			categories["Logs"] = append(categories["Logs"], binding)
		}
	}
...
// generateQuickReference generates a quick reference section
func (h *HelpComponent) generateQuickReference() string {
	var content strings.Builder
	
	content.WriteString(h.CategoryStyle.Render("Quick Reference"))
	content.WriteString("\n")
	...
	for _, item := range quickRef {
		keyPart := h.KeyStyle.Render(item.key)
		descPart := h.DescriptionStyle.Render(item.desc)
